...
public class ExtendedChainBuilder<TRequest> : IChainBuilder<TRequest> where TRequest : notnull
{
    ...
    /// <summary>
    /// Inizializza una nuova istanza della classe <see cref="ExtendedChainBuilder{TRequest}"/>.
    /// </summary>
    ...
    public ExtendedChainBuilder(IServiceProvider serviceProvider, IEnumerable<Type> handlerTypes, ChainConfiguration configuration)
    {
    ...
    public IChainHandler<TRequest> Build()
    {
        ...
        {
            // Se è stata richiesta ReturnEmpty, crea un handler vuoto
            if (_configuration.MissingHandlerBehavior == MissingHandlerBehavior.ReturnEmpty)
            {
                // Creare un'implementazione anonima di IChainHandler<TRequest> che non fa nulla
                return new EmptyChainHandler<TRequest>();
            }
            
            handlerInstances = ChainHelpers.HandleMissingHandlers<IChainHandler<TRequest>>(
        ...
        return handlerInstances.Count > 0 ? handlerInstances[0] : null!;
    }
}
...
public class ExtendedChainBuilder<TRequest, TResponse> : IChainBuilder<TRequest, TResponse> where TRequest : notnull
{
    private readonly IServiceProvider _serviceProvider;
    private readonly IEnumerable<Type> _handlerTypes;
    private readonly ChainConfiguration _configuration;
    ...
    /// <summary>
    /// Inizializza una nuova istanza della classe <see cref="ExtendedChainBuilder{TRequest, TResponse}"/>.
    /// </summary>
    /// <param name="serviceProvider">Il provider di servizi.</param>
    /// <param name="handlerTypes">I tipi di handler da includere nella catena.</param>
    /// <param name="configuration">La configurazione della catena.</param>
    ...
    public IChainHandler<TRequest, TResponse> Build()
    {
        ...
        if (handlerInstances.Count == 0)
        {
            handlerInstances = ChainHelpers.HandleMissingHandlers<IChainHandler<TRequest, TResponse>>(
                _configuration.MissingHandlerBehavior, 
                typeof(TRequest));
            
            // Se la lista è ancora vuota ed è stata richiesta ReturnEmpty, restituisci un handler vuoto
            if (handlerInstances.Count == 0 && _configuration.MissingHandlerBehavior == MissingHandlerBehavior.ReturnEmpty)
            {
